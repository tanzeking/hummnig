...
======================================================================================
Hummingbot 是一个开源软件客户端，帮助您构建和运行
做市、套利和其他高频交易机器人。

- 官方代码库: https://github.com/hummingbot/hummingbot
- 加入社区: https://discord.gg/hummingbot
- 学习做市: https://hummingbot.org/botcamp
...
def create_search_field() -> SearchToolbar:
    return SearchToolbar(text_if_not_searching=[('class:primary', "[CTRL + F] 开始搜索。")],
                         forward_search_prompt=[('class:primary', "搜索日志 [按 CTRL + F 隐藏搜索] >>> ")],
                         ignore_case=True)
...
def get_version():
    return [("class:header", f"版本号: {version}")]
...
def get_active_strategy():
    ...
    style = "class:log_field"
    return [(style, f"策略: {hb.strategy_name}")]
...
def get_strategy_file():
    ...
    hb = HummingbotApplication.main_application()
    style = "class:log_field"
    return [(style, f"策略文件: {hb.strategy_file_name}")]
...
def get_gateway_status():
    ...
    return [(style, f"{lock_icon}网关: {status_display}")]
