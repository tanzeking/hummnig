...
class HummingbotCompleter(Completer):
    ...
    def _complete_exchanges(self, document: Document) -> bool:
        return any(x for x in ("exchange name", "name of exchange", "name of the exchange", "交易所名称", "填写交易所")
    ...
    def _complete_gateway_network_selection(self, document: Document) -> bool:
        return ("Which" in self.prompt_text and "network do you want to connect to?" in self.prompt_text) or \
               ("想要连接哪个网络" in self.prompt_text)
    ...
    def _complete_gateway_chain(self, document: Document) -> bool:
        return "Which chain do you want" in self.prompt_text or "你想要哪条链" in self.prompt_text or \
    ...
    def _complete_gateway_network(self, document: Document) -> bool:
        return "Which network do you want" in self.prompt_text or "你想要哪个网络" in self.prompt_text

    def _complete_gateway_wallet_addresses(self, document: Document) -> bool:
        return "Select a gateway wallet" in self.prompt_text or "选择一个网关钱包" in self.prompt_text
